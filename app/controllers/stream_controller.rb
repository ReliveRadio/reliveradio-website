...
class StreamController < ApplicationController
  ...
  def listeners_technique
    @listeners = fetch_listeners("Technik")
    respond_to do |format|
      format.js { render 'listeners'}
    end
  end
  ...
  def listeners_culture
    @listeners = fetch_listeners("Kultur")
    respond_to do |format|
      format.js { render 'listeners'}
    end
  end
  ...
  def render_genre(genre_name, airtime_url)
    ...
    @listeners = fetch_listeners(genre_name)
    # fetch really live podcasts
    @live_podcasts = fetch_hoersuppe_livepodcasts
    # fetch episode schedule
    @episodes = fetch_episode_schedule(airtime_url)
